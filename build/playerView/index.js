...
export default function CommonVideo(props) {
    const { height: propHeight = 250, showGG = false, ggUrl = '', url = '', showBack = false, showTitle = false, widthCamera, isFull } = props;
    if (widthCamera)
        deviceWidth = widthCamera;
    ...
    const _onLayout = (e) => {
        let { width: layoutWidth, height: layoutHeight } = e.nativeEvent.layout;
    ...
    let { onGGEnd, onEnd, style, title, onLeftPress, videoAspectRatio = null, fullVideoAspectRatio = null } = props;
    let currVidAspectRatio = null;
    if (isFull) {
    ...
    , { 
        ...
        showTop && React.createElement(View, { style: styles.topView },
            React.createElement(View, { style: styles.backBtn },
                ...
                React.createElement(View, { style: { justifyContent: 'center', flex: 1, marginRight: 10 } }, showTitle &&
                    React.createElement(Text, { style: { color: '#fff', fontSize: 16 }, numberOfLines: 1 }, title)))),
        realShowGG && (React.createElement(VLCPlayerView, { ...props, videoAspectRatio: currentVideoAspectRatio, uri: ggUrl, source: { uri: ggUrl, type: ggType }, type: ggType, isGG: true, showBack: showBack, showTitle: showTitle, isFull: isFull, onEnd: () => {
                onGGEnd && onGGEnd();
                setIsEndGG(true);
            }, startFullScreen: toFullScreen, closeFullScreen: closeFullScreen })),
        showVideo && (React.createElement(VLCPlayerView, { ...props, uri: currentUrl, videoAspectRatio: currentVideoAspectRatio, onLeftPress: onLeftPress, title: title, type: type, isFull: isFull, showBack: showBack, showTitle: showTitle, isGG: true, 
            // isEndGG={isEndGG}
            //initPaused={this.state.paused}
            style: showGG && !isEndGG ? { position: 'absolute', zIndex: -1 } : {}, source: { uri: currentUrl, type: type }, startFullScreen: toFullScreen, closeFullScreen: closeFullScreen, onEnd: () => {
                onEnd && onEnd();
            } }))));
}
