...
import { StyleSheet, Text, View, Dimensions, TouchableOpacity, ActivityIndicator, 
// StatusBar,
// BackHandler,
// Modal,
...
export default function VLCPlayerView(props) {
    // static propTypes = {
    ...
    const { initPaused = false, source = null, seek = 0, playInBackground = false, isGG = false, autoplay = true, errorTitle = 'Error play video, please try again',
    // isFull
     } = props;
    ...
    const _onOpen = (e) => console.log('onOpen', e);
    ...
    const { onEnd, style, type, isFull: propIsFull, uri, title, onLeftPress, closeFullScreen, showBack, showTitle, videoAspectRatio = '16:9', showGoLive, onGoLivePress, onReplayPress, titleGolive, showLeftButton, showMiddleButton, showRightButton, } = props;
    ...
    return (React.createElement(TouchableOpacity, { activeOpacity: 1, style: [styles.videoBtn, style], onPressOut: () => {
            let currentTime = new Date().getTime();
        ...
        React.createElement(VLCPlayer
        // ref={vlcPlayer}
        , { 
            // ref={vlcPlayer}
            paused: paused, 
            //seek={this.state.seek}
            style: [styles.video], source: source, videoAspectRatio: videoAspectRatio, onProgress: onProgress, onEnded: onEnded, 
            //onEnded={this.onEnded.bind(this)}
            onStopped: onEnded, onPlaying: onPlaying, onVideoBuffering: onBuffering, onPaused: onPaused, progressUpdateInterval: 250, onError: _onError, onVideoOpen: _onOpen, onVideoLoadStart: _onLoadStart }),
        realShowLoding &&
            !isError && (React.createElement(View, { style: styles.loading },
            React.createElement(ActivityIndicator, { size: 'large', animating: true, color: "#fff" }))),
        isError && (React.createElement(View, { style: [styles.loading, { backgroundColor: '#000' }] },
            React.createElement(Text, { style: { color: 'red' } }, errorTitle),
            React.createElement(TouchableOpacity, { activeOpacity: 1, onPress: _reload, style: {
                    width: 100,
                    alignItems: 'center',
                    justifyContent: 'center',
                    marginTop: 10,
                } },
                React.createElement(Icon, { name: 'reload', size: 45, color: "#fff" })))),
        React.createElement(View, { style: styles.topView },
            React.createElement(View, { style: styles.backBtn },
                showBack && (React.createElement(TouchableOpacity, { onPress: () => {
                        if (isFull) {
                            closeFullScreen && closeFullScreen();
                        }
                        else {
                    ...
                    React.createElement(Icon, { name: 'chevron-left', size: 30, color: "#fff" }))),
                React.createElement(View, { style: { justifyContent: 'center', flex: 1, marginRight: 10 } }, showTitle &&
                    showControls && (React.createElement(Text, { style: { color: '#fff', fontSize: 16 }, numberOfLines: 1 }, title))),
                showGG && (React.createElement(View, { style: styles.GG },
                    React.createElement(TimeLimt, { onEnd: () => {
                            onEnd && onEnd();
                        } }))))),
        React.createElement(View, { style: [styles.bottomView] }, showControls && (React.createElement(ControlBtn
        //style={isFull?{width:deviceHeight}:{}}
        , { 
            //style={isFull?{width:deviceHeight}:{}}
            showSlider: !isGG, showGG: showGG, onEnd: onEnd, title: title, onLeftPress: onLeftPress, paused: paused, isFull: isFull, currentTime: currentTime, totalTime: totalTime, onPausedPress: _play, onFullPress: _toFullScreen, onValueChange: value => {
            ...
            }, showGoLive: showGoLive, onGoLivePress: onGoLivePress, onReplayPress: onReplayPress, titleGolive: titleGolive, showLeftButton: showLeftButton, showMiddleButton: showMiddleButton, showRightButton: showRightButton })))));
}
